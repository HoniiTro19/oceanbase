...
int ObMySQLConnection::execute_write_async(ObMySQLPreparedStatement &stmt)
{
  ...
  if (OB_UNLIKELY(closed_)) {
    ...
    LOG_DEBUG("async update started", "server_addr", get_server(), K(ret));
  }
  return ret;
}
...
int ObMySQLConnection::execute_read(ObMySQLPreparedStatement &stmt, ObISQLClient::ReadResult &res) 
{
  ...
  if (OB_UNLIKELY(closed_)) {
    ...
    LOG_DEBUG("query succeed", "server_addr", get_server(), K(ret));
  }
  return ret;
}
...
int ObMySQLConnection::execute_read_async(ObMySQLPreparedStatement &stmt)
{
  ...
  if (OB_UNLIKELY(closed_)) {
    ...
    mysql_stmt_ = stmt.get_stmt_handler();
    LOG_DEBUG("async query started", "server_addr", get_server(), K(ret));
  }
  return ret;
}
...
int ObContFunc::update_begin_status()
{
  int ret = OB_SUCCESS;
...
int ObContFunc::update_commit_status()
{
  int ret = OB_SUCCESS;
