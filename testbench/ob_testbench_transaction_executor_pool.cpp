...
ObTestbenchTransactionExecutorPool::ObTestbenchTransactionExecutorPool() 
  : tg_id_(-1), is_inited_(false), sql_proxy_(NULL), success_task_(0), failure_task_(0) {}
...
void ObTestbenchTransactionExecutorPool::handle(void *task) {
  ...
  // always clear transaction and connection status
  if (OB_FAIL(task_to_handle->release_dblinks())) {
...
void ObTestbenchTransactionExecutorPool::inc_success_task() {
  ATOMIC_INC(&success_task_);
}

void ObTestbenchTransactionExecutorPool::inc_failure_task() {
  ATOMIC_INC(&failure_task_);
}
} // namespace testbench
} // namespace oceanbase